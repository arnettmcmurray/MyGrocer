...
def _list_row(gl: GroceryList):
    ...
    return {"id": gl.id, "name": gl.name, "is_active": gl.is_active, "items": [{"id": i.id, "source_item_id": i.source_item_id, "quantity": i.quantity, "checked": i.checked} for i in items]}
@bp.get("/active")
@jwt_required()
def get_active():
...
def check_item():
    ...
    gl = _get_active_list(uid)
    gli = GroceryListItem.query.join(GroceryList).filter(GroceryList.id == gl.id, GroceryListItem.id == item_id).first()
    if not gli:
